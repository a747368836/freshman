package com.mredrock.cyxbs.freshman.Tools.FixedCardview
    //goto 386 设置delta(阴影的上下位置 -1 ~ 1  -1是最上边 )
    // 436 把改delta的注释掉了 这玩意后期要用再加上
...
import com.mredrock.cyxbs.freshman.Tools.FixedCardview.CardViewApi21Impl
import com.mredrock.cyxbs.freshman.Tools.FixedCardview.CardViewApi17Impl
import com.mredrock.cyxbs.freshman.Tools.FixedCardview.CardViewImpl
...
class CardView : FrameLayout {
    ...
    private val mCardViewDelegate = object : CardViewDelegate {
        ...
        override fun setShadowPadding(left: Int, top: Int, right: Int, bottom: Int) {
            mShadowBounds.set(left+DisplayUtils.dp2px(context,15f), top, right+DisplayUtils.dp2px(context,15f), bottom)
            super@CardView.setPadding(DisplayUtils.dp2px(context,15f), top + mContentPadding.top,
                    DisplayUtils.dp2px(context,15f), bottom + mContentPadding.bottom)
        }
    ...
    private var mTopDelta: Float=(0f)
    ...
    private fun initialize(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        if (a2.hasValue(R.styleable.CardViewShadow_endColor) && a2.hasValue(R.styleable.CardViewShadow_startColor)) {
            ...
            IMPL.initialize(mCardViewDelegate, context, backgroundColor, radius,
                    elevation, maxElevation,resources.getColor(R.color.freshman_card_startcolor),resources.getColor(R.color.freshman_card_endcolor),mTopDelta)
        } else {
            IMPL.initialize(mCardViewDelegate, context, backgroundColor, radius,
                    elevation, maxElevation,resources.getColor(R.color.freshman_card_startcolor),resources.getColor(R.color.freshman_card_endcolor),mTopDelta)
        }
